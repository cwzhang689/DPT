...
from src import model as mm  # 导入模型模块
import torchvision
print(torchvision.__version__)
print(torch.cuda.is_available())  # 如果返回 True，说明 CUDA 可用
print(torch.cuda.get_device_name(0))  # 获取第一个 CUDA 设备的名称
...
hyp_params = args
hyp_params.use_cuda = torch.cuda.is_available() and not args.no_cuda
...
if args.pretrained_model:
    ...
    model = transfer_model(model, args.pretrained_model, hyp_params.use_cuda)
else:
    # 否则，创建新的模型实例
